...
The `--ws <name>` flag scopes RapidX operations to a specific workstream, enabling
parallel milestone work by multiple Claude Code instances on the same codebase.

## Resolution Priority

1. `--ws <name>` flag (explicit, highest priority)
2. `RAPIDX_WORKSTREAM` environment variable (per-instance)
...
RapidX now prefers a session-scoped pointer keyed by runtime/session identity
(`RAPIDX_SESSION_KEY`, `CODEX_THREAD_ID`, `CLAUDE_CODE_SSE_PORT`, terminal session IDs,
...
When RapidX resolves the session-scoped pointer in step 3 above, it uses this order:

1. Explicit runtime/session env vars such as `RAPIDX_SESSION_KEY`, `CODEX_THREAD_ID`,
...
If none of those produce a stable identity, RapidX does not keep probing. It falls
...
non-interactive, RapidX intentionally skips shelling out to `tty` because that path
...
- If that was the last pointer for the repo, RapidX also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, RapidX
...
RapidX does not currently run a background garbage collector for historical temp
...
All workflow routing commands include `${RAPIDX_WS}` which:
...
```bash
# All rapidx-sdk query commands accept --ws
rapidx-sdk query state.json --ws feature-a
rapidx-sdk query find-phase 3 --ws feature-b

# Session-local switching without --ws on every command
RAPIDX_SESSION_KEY=my-terminal-a rapidx-sdk query workstream.set feature-a
RAPIDX_SESSION_KEY=my-terminal-a rapidx-sdk query state.json
RAPIDX_SESSION_KEY=my-terminal-b rapidx-sdk query workstream.set feature-b
RAPIDX_SESSION_KEY=my-terminal-b rapidx-sdk query state.json

# Workstream CRUD
rapidx-sdk query workstream.create <name>
rapidx-sdk query workstream.list
rapidx-sdk query workstream.status <name>
rapidx-sdk query workstream.complete <name>
```
