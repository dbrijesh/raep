...
**Input:** Must-have artifact: `get-things-done/workflows/context-bridge.md: >50 lines`
...
> **Method:** Ran `grep -n "scope" get-things-done/bin/validate-commit.js` to locate scope validation logic.
...
> **Ran:** `echo "feat(: test" | node get-things-done/bin/validate-commit.js` -- exit code 0 (accepted).
...
> **Method:** Ran `grep -n "feedback-loop\|extractFeedback" get-things-done/bin/check-subagent-output.js`.
...
> **Cross-check:** `grep -rn "require.*feedback-loop" get-things-done/bin/` returns zero results across all scripts.
...
> **Method:** Ran `grep "architecture_graph\|architecture_guard" get-things-done/bin/lib/config-schema.json`.
> **Evidence:** Zero matches in schema. However, `grep "architecture" get-things-done/bin/lib/config.cjs` shows both flags in CONFIG_DEFAULTS (lines 426, 428).
...
**Input:** Must-have artifact: `get-things-done/bin/lib/audit-checks/index.cjs: >100 lines`

**Output:**

> **L1 (Existence):** EXISTS -- File found at `get-things-done/bin/lib/audit-checks/index.cjs`, 142 lines.
> **Status: PASSED**
