...
```bash
node "$HOME/.claude/get-things-done/bin/gsd-tools.cjs" websearch "your query" --limit 10
```
...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
```bash
node "$HOME/.claude/get-things-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
