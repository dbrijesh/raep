...
- `failures`: (fix mode only) array of `{line, claim, expected, actual}` objects from rapidx-doc-verifier output
...
<fix_mode>
Correct specific failing claims identified by the rapidx-doc-verifier. ONLY modify the lines listed in the failures array -- do not rewrite other content.
