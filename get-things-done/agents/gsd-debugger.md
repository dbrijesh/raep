...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gsd-tools.cjs" state load)
...
```bash
node "$HOME/.claude/get-things-done/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```markdown
# Get Things Done Debug Knowledge Base
...
```bash
node "$HOME/.claude/get-things-done/bin/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
