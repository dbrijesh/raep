...
@~/.claude/get-things-done/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@~/.claude/get-things-done/references/ios-scaffold.md
...
@~/.claude/get-things-done/references/executor-examples.md
</deviation_rules>
...
```bash
rapidx-sdk query commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
<state_updates>
After SUMMARY.md, update STATE.md using `rapidx-sdk query` state handlers (positional args; see `sdk/src/query/QUERY-HANDLERS.md`):
...
for decision in "${DECISIONS[@]}"; do
  rapidx-sdk query state.add-decision "${decision}"
done
...
rapidx-sdk query roadmap.update-plan-progress "${PHASE_NUMBER}"
...
```bash
rapidx-sdk query state.add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
rapidx-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" \
