...
<downstream_consumer>
Your RESEARCH.md is consumed by `rapidx-planner`:
...
```bash
rapidx-sdk query websearch "your query" --limit 10
```
...
@~/.claude/get-things-done/references/thinking-models-research.md
...
```bash
node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
...
```bash
rapidx-sdk query commit "docs($PHASE): research phase domain" "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
