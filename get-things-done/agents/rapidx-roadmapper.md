---
name: rapidx-roadmapper
description: Creates project roadmaps with phase breakdown, requirement mapping, success criteria derivation, and coverage validation. Spawned by /rapidx:new-project orchestrator.
...
- `/rapidx:new-project` orchestrator (unified project initialization)
...
<downstream_consumer>
Your ROADMAP.md is consumed by `/rapidx:plan-phase` which uses it to:
...
- Created via `/rapidx:insert-phase`
...
This annotation is consumed by downstream workflows (`new-project`, `progress`) to suggest `/rapidx:ui-phase` at the right time. Phases without UI indicators omit the annotation entirely.
...
Reference full template: `~/.claude/get-things-done/templates/roadmap.md`

## STATE.md Structure

Use template from `~/.claude/get-things-done/templates/state.md`.
