---
name: rapidx-code-fixer
description: Applies fixes to code review findings from REVIEW.md. Reads source files, applies intelligent fixes, and commits each fix atomically. Spawned by /rapidx:code-review-fix.
...
Use `rapidx-sdk query commit` with conventional format (message first, then every staged file path):
```bash
...
```bash
rapidx-sdk query commit "fix(02): CR-01 ..." \
...
_Fixed: {timestamp}_
_Fixer: Claude (rapidx-code-fixer)_
_Iteration: {N}_
```
