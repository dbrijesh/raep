---
name: rapidx-ui-checker
description: Validates UI-SPEC.md design contracts against 6 quality dimensions. Produces BLOCK/FLAG/PASS verdicts. Spawned by /rapidx:ui-phase orchestrator.
...
<upstream_input>
**UI-SPEC.md** — Design contract from rapidx-ui-researcher (primary input)
...
fix_hint: "Re-run /rapidx:ui-phase to trigger the registry vetting gate, or manually run 'npx shadcn view {block} --registry {url}' and record results"
```
```yaml
dimension: 6
severity: PASS
...
### Action Required
Fix blocking issues in UI-SPEC.md and re-run `/rapidx:ui-phase`.
```
