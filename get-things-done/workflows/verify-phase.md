...
<required_reading>
@~/.claude/get-things-done/references/verification-patterns.md
@~/.claude/get-things-done/templates/verification-report.md
</required_reading>
...
```bash
rapidx-sdk query roadmap.analyze
```
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(rapidx-sdk query frontmatter.get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(rapidx-sdk query roadmap.get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(rapidx-sdk query verify.artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(rapidx-sdk query verify.key-links "$plan")
...
See ~/.claude/get-things-done/templates/verification-report.md for complete template.
</step>
