...
```bash
INIT=$(rapidx-sdk query init.ingest-docs 2>/dev/null || rapidx-sdk query init.default)
```
...
For each discovered doc, spawn `rapidx-doc-classifier` in parallel. In Claude Code, issue all Task calls in a single message with multiple tool uses so the harness runs them concurrently. For Copilot / sequential runtimes, fall back to sequential dispatch.
...
Spawn `rapidx-doc-synthesizer` once:

```
Task({
  subagent_type: "rapidx-doc-synthesizer",
  prompt: "
...
```bash
rapidx-sdk query commit "docs: ingest {N} docs from {SCAN_PATH} (#2387)" \
  .planning/PROJECT.md \
  .planning/REQUIREMENTS.md \
  .planning/ROADMAP.md \
  .planning/STATE.md \
