...
@~/.claude/get-things-done/references/git-integration.md
</required_reading>
...
```bash
INIT=$(rapidx-sdk query init.execute-phase "${PHASE}")
...
# config settings can be fetched via rapidx-sdk query config-get if needed
```
...
INLINE_THRESHOLD=$(rapidx-sdk query config-get workflow.inline_plan_threshold 2>/dev/null || echo "2")
...
2. Per segment:
   - Subagent route: spawn rapidx-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
rapidx-sdk query phases.list --type summaries --raw
...
Apply deviation rules from the rapidx-executor agent definition (single source of truth):
...
See `~/.claude/get-things-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/get-things-done/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(rapidx-sdk query config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/get-things-done/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/get-things-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/get-things-done/templates/summary.md`.
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  rapidx-sdk query state.advance-plan

  # Recalculate progress bar from disk state
  rapidx-sdk query state.update-progress

  # Record execution metrics
  rapidx-sdk query state.record-metric \
...
rapidx-sdk query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
rapidx-sdk query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
if [ "$IS_WORKTREE" != "true" ]; then
  rapidx-sdk query roadmap.update-plan-progress "${PHASE}"
fi
```
...
```bash
rapidx-sdk query requirements.mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  rapidx-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  rapidx-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
...
```bash
rapidx-sdk query commit "" .planning/codebase/*.md --amend
```
</step>
