...
- rapidx-code-reviewer — Reviews source files for bugs, security issues, and code quality
</available_agent_types>
...
```bash
INIT=$(rapidx-sdk query init.quick "$DESCRIPTION")
...
Spawn rapidx-executor with plan reference:
...
Task(
  ...
  Depth: quick",
  subagent_type="rapidx-code-reviewer",
  model="{executor_model}"
)
```
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `rapidx-sdk query commit` command (or legacy `rapidx-tools.cjs` commit) handles already-committed files gracefully.
...
rapidx-sdk query commit "docs(quick-${quick_id}): ${DESCRIPTION}" ${file_list}
```
