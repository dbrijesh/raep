...
```bash
INIT=$(rapidx-sdk query docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(rapidx-sdk query agent-skills rapidx-doc-writer 2>/dev/null)
```
...
- These will be passed to rapidx-doc-verifier in the verify_docs step for accuracy checking
- If inaccuracies are found, they will be dispatched to rapidx-doc-writer in `fix` mode for surgical corrections
...
4. For each gap the user selects:
   ...
   - The rapidx-doc-writer will receive a `doc_assignment` with `type: "custom"` and a description of what to document, using the project's source files as content discovery targets
...
Spawn 3 parallel rapidx-doc-writer agents for Wave 1 docs: README, ARCHITECTURE, CONFIGURATION.
...
```
Task(
  subagent_type="rapidx-doc-writer",
...
Spawn a `rapidx-doc-writer` agent with `run_in_background=true`:
...
**Fallback when Task tool is unavailable:** Generate per-package READMEs sequentially inline after the `sequential_generation` step. For each package directory with a `package.json`, construct the equivalent `doc_assignment` block and generate the README following rapidx-doc-writer instructions.
...
1. **README** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase (Read, Grep, Glob, Bash) following rapidx-doc-writer create_mode / update_mode instructions
...
2. **ARCHITECTURE** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase following rapidx-doc-writer instructions
...
- Follow rapidx-doc-writer instructions for per_package scope
...
1. Spawn the `rapidx-doc-verifier` agent (or invoke sequentially if Task tool is unavailable) with a `<verify_assignment>` block:
   ```xml
   <verify_assignment>
...
1. Spawn the `rapidx-doc-verifier` agent with the same `<verify_assignment>` block as above.
...
Non-canonical docs with failures ARE eligible for the fix_loop. When a non-canonical doc has `claims_failed > 0`, dispatch it to rapidx-doc-writer in `fix` mode with the failures array — the writer's fix mode does surgical corrections on specific lines regardless of doc type (no template needed). The writer MUST NOT restructure, rephrase, or reformat any content beyond the failing claims.
...
1. For each doc with `claims_failed > 0` in the latest verification_results:
   ...
   b. Spawn `rapidx-doc-writer` agent (or invoke sequentially) with a fix assignment:
      ```xml
      <doc_assignment>
...
Invoke the rapidx-doc-verifier agent in read-only mode for each file in `existing_docs` from the init JSON:

1. For each doc in `existing_docs`:
   a. Spawn `rapidx-doc-verifier` (or invoke sequentially if Task tool is unavailable) with:
      ```xml
      <verify_assignment>
      doc_path: {doc.path}
      project_root: {project_root from init JSON}
...
```bash
rapidx-sdk query commit "docs: generate project documentation" \
...
- [ ] --verify-only invokes rapidx-doc-verifier for full fact-checking (not just VERIFY marker count)
</success_criteria>
