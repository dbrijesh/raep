<purpose>
Auto-fix issues from REVIEW.md. Validates phase, checks config gate, verifies REVIEW.md exists and has fixable issues, spawns rapidx-code-fixer agent, handles --auto iteration loop (capped at 3), commits REVIEW-FIX.md once at the end, and presents results.
</purpose>
...
- rapidx-code-fixer: Applies fixes to code review findings
- rapidx-code-reviewer: Reviews source files for bugs and issues
</available_agent_types>
...
if [ ! -f "${REVIEW_PATH}" ]; then
  echo "Error: No REVIEW.md found for Phase ${PHASE_ARG}. Run /rapidx:code-review ${PHASE_ARG} first."
  exit 1
fi
```
...
Spawn the rapidx-code-fixer agent with config:
...
```
Task(subagent_type="rapidx-code-fixer", prompt="
<files_to_read>
${REVIEW_PATH}
</files_to_read>
...
You can retry with /rapidx:code-review-fix ${PHASE_ARG}.
```
...
if [ "$AUTO_MODE" = "true" ]; then
  ...
  - ${f}"
    ...
    # Spawn rapidx-code-reviewer agent to re-review
...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: /rapidx:code-review-fix ${PHASE_ARG}"
  echo ""
  echo "═══════════════════════════════════════════════════════════════"
  exit 1
fi
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  /rapidx:verify-work  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  /rapidx:code-review ${PHASE_NUMBER}           — Re-review code"
  echo "  /rapidx:verify-work                           — Verify phase completion"
  echo ""
fi
```
