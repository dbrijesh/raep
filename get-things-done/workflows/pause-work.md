<purpose>
Create structured `.planning/HANDOFF.json` and `.continue-here.md` handoff files to preserve complete work state across sessions. The JSON provides machine-readable state for `/rapidx:resume-work`; the markdown provides human-readable context.
</purpose>
...
```bash
timestamp=$(rapidx-sdk query current-timestamp full --raw)
```
...
<step name="commit">
```bash
rapidx-sdk query commit "wip: [context-name] paused at [X]/[Y]" [handoff-path] .planning/HANDOFF.json
```
</step>
