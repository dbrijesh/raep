<purpose>
Lightweight codebase assessment. Spawns a single rapidx-codebase-mapper agent for one focus area,
...
```bash
INIT=$(rapidx-sdk query init.map-codebase 2>/dev/null || echo "{}")
...
Spawn a single `rapidx-codebase-mapper` agent with the selected focus area:
