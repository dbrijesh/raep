...
<required_reading>
@~/.claude/get-things-done/references/domain-probes.md
@~/.claude/get-things-done/references/gate-prompts.md
@~/.claude/get-things-done/references/universal-anti-patterns.md
</required_reading>

<downstream_awareness>
**CONTEXT.md feeds into:**

1. **rapidx-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **rapidx-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
- Read and execute @~/.claude/get-things-done/workflows/discuss-phase-power.md end-to-end
...
```bash
TODO_MATCHES=$(rapidx-sdk query todo.match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(rapidx-sdk query resolve-model rapidx-advisor-researcher --raw)
   ```
...
```bash
PROFILE_CONTENT=$(cat "$HOME/.claude/get-things-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
```bash
MAX_PASSES=$(rapidx-sdk query config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
rapidx-sdk query commit "docs(${padded_phase}): capture phase context" "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
rapidx-sdk query commit "docs(state): record phase ${PHASE} context session" .planning/STATE.md
```
</step>
...
**Full step-by-step instructions:** @~/.claude/get-things-done/workflows/discuss-phase-power.md
