...
AGENT_SKILLS_ANALYZER=$(rapidx-sdk query agent-skills rapidx-assumptions-analyzer 2>/dev/null)
```
...
- When spawning the rapidx-assumptions-analyzer, pass the lens list so it can flag which lenses apply
...
Spawn a `rapidx-assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```
Task(subagent_type="rapidx-assumptions-analyzer", prompt="""
...
```bash
rapidx-sdk query commit "docs(${padded_phase}): capture phase context (assumptions mode)" "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
