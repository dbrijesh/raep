...
2. **Find next backlog number:**
   ```bash
   NEXT=$(node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" phase next-decimal 999 --raw)
   ```
...
5. **Commit:**
   ```bash
   node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" commit "docs: add backlog item ${NEXT} — ${ARGUMENTS}" --files .planning/ROADMAP.md ".planning/phases/${NEXT}-${SLUG}/.gitkeep"
   ```
