...
**CJS-only:** `from-gsd2` is not on the `rapidx-sdk query` registry; call `rapidx-tools.cjs` as shown below (see `docs/CLI-TOOLS.md`).
</objective>
...
1. **Locate the .gsd/ directory** — check the current working directory (or `--path` argument):
   ```bash
   node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" from-gsd2 --dry-run
   ```
...
3. **Run the migration** after confirmation:
   ```bash
   node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" from-gsd2
   ```
