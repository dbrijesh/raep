...
**CJS-only (graphify):** `graphify` subcommands are not registered on `rapidx-sdk query`. Use `node $HOME/.claude/get-things-done/bin/rapidx-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `rapidx-sdk query` where a handler exists.
...
```bash
node $HOME/.claude/get-things-done/bin/rapidx-tools.cjs graphify query <term>
```
...
```bash
node $HOME/.claude/get-things-done/bin/rapidx-tools.cjs graphify diff
```
...
```
PREFLIGHT=$(node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" graphify build)
```
...
4. **Write diff snapshot:**
   ```
   node \"$HOME/.claude/get-things-done/bin/rapidx-tools.cjs\" graphify build snapshot
   ```
