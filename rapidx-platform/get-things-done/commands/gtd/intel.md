...
```bash
rapidx-sdk query intel.query <term>
```
...
Task(
  ...
  prompt="You are the rapidx-intel-updater agent. Your job is to analyze this codebase and write/update intelligence files in .planning/intel/.

Project root: ${CWD}
Prefer: rapidx-sdk query <subcommand> (installed rapidx-sdk on PATH). Legacy: node $HOME/.claude/get-things-done/bin/rapidx-tools.cjs
...
4. Use `rapidx-sdk query intel.extract-exports <file>` to analyze source files
5. Use `rapidx-sdk query intel.patch-meta <file>` to update timestamps after writing
6. Use `rapidx-sdk query intel.validate` to check your output
