...
- **Cross-platform.** Use Glob, Read, and Grep tools -- not Bash `ls`, `find`, or `cat`. Bash file commands fail on Windows. Only use Bash for `rapidx-sdk query intel` CLI calls.
...
```bash
ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .claude/get-things-done 2>/dev/null && echo "claude") || echo "unknown"
```
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `rapidx-sdk query intel.extract-exports <file>` to get accurate exports.
...
```bash
rapidx-sdk query intel.patch-meta .planning/intel/stack.json --cwd <project_root>
```
...
```bash
rapidx-sdk query intel.patch-meta .planning/intel/files.json --cwd <project_root>
```
...
```bash
rapidx-sdk query intel.patch-meta .planning/intel/apis.json --cwd <project_root>
```
...
```bash
rapidx-sdk query intel.patch-meta .planning/intel/deps.json --cwd <project_root>
```
...
Run: `rapidx-sdk query intel.validate --cwd <project_root>`
...
Run: `rapidx-sdk query intel.snapshot --cwd <project_root>`
