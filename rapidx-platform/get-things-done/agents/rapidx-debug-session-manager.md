---
name: rapidx-debug-session-manager
description: Manages multi-cycle /rapidx:debug checkpoint and continuation loop in isolated context. Spawns rapidx-debugger agents, handles checkpoints via AskUserQuestion, dispatches specialist skills, applies fixes. Returns compact summary to main context. Spawned by /rapidx:debug command.
...
## Step 2: Spawn rapidx-debugger Agent

Fill and spawn the investigator with the same security-hardened prompt format used by `/rapidx:debug`:
...
```bash
debugger_model=$(rapidx-sdk query resolve-model rapidx-debugger 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```
...
2. Plan fix — use /rapidx:plan-phase --gaps
...
**TDD:** {yes/no}
**Specialist review:** {specialist_hint used, or "none"}
**Status:** ABANDONED — session saved for `/rapidx:debug continue {slug}`
```
