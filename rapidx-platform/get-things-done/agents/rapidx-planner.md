...
**Project skills:** @~/.claude/get-things-done/references/project-skills-discovery.md
...
@~/.claude/get-things-done/references/planner-source-audit.md for full format, examples, and gap-handling rules.
...
@~/.claude/get-things-done/references/planner-source-audit.md for constraint examples.
...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @~/.claude/get-things-done/references/planner-antipatterns.md for vague-vs-specific comparison table.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @~/.claude/get-things-done/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
@~/.claude/get-things-done/references/planner-antipatterns.md
...
<gap_closure_mode>
See `get-things-done/references/planner-gap-closure.md`. Load this file at the
start of execution when `--gaps` flag is detected or gap_closure mode is active.
</gap_closure_mode>

<revision_mode>
See `get-things-done/references/planner-revision.md`. Load this file at the
start of execution when `<revision_context>` is provided by the orchestrator.
</revision_mode>

<reviews_mode>
See `get-things-done/references/planner-reviews.md`. Load this file at the
...
- If `--gaps` flag or gap_closure context present: Read `get-things-done/references/planner-gap-closure.md`
- If `<revision_context>` provided by orchestrator: Read `get-things-done/references/planner-revision.md`
- If `--reviews` flag present or reviews mode active: Read `get-things-done/references/planner-reviews.md`
...
```bash
node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" graphify status
```
...
```bash
node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" graphify query "<phase-goal-keyword>" --budget 2000
```

(graphify is not exposed on `rapidx-sdk query` yet; use `rapidx-tools.cjs` for graphify only.)
...
```bash
rapidx-sdk query history-digest
```
...
@~/.claude/get-things-done/references/thinking-models-planning.md
...
Validate each created PLAN.md using `rapidx-sdk query`:

```bash
VALID=$(rapidx-sdk query frontmatter.validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(rapidx-sdk query verify.plan-structure "$PLAN_PATH")
```
...
```bash
rapidx-sdk query commit "docs($PHASE): create phase plan" \
