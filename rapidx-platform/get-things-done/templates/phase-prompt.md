...
> **Note:** Planning methodology is in `agents/rapidx:planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/get-things-done/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/rapidx:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/get-things-done/references/tdd.md` for TDD plan structure.
...
See `~/.claude/get-things-done/templates/user-setup.md` for full schema and examples
...
See `~/.claude/get-things-done/workflows/verify-phase.md` for verification logic.
