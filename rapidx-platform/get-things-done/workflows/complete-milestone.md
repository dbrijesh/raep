...
`audit-open` is not registered on `rapidx-sdk query` yet; use the installed CJS CLI:

```bash
node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" audit-open 2>/dev/null
```
...
[R] Resolve — stop and fix items, then re-run /rapidx:complete-milestone
...
SECURITY: Audit JSON output is structured data from `audit-open` (rapidx-tools.cjs) — validated and sanitized at source. When writing to STATE.md, item slugs and descriptions are sanitized via `sanitizeForDisplay()` before inclusion. Never inject raw user-supplied content into STATE.md without sanitization.
</step>
...
2. **Run audit first** — `/rapidx:audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  rapidx-sdk query summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `rapidx-sdk query milestone.complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to `rapidx-sdk query milestone.complete`:**

```bash
ARCHIVE=$(rapidx-sdk query milestone.complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/rapidx:cleanup` later to archive retroactively.
...
```bash
rapidx-sdk query commit "chore: archive v[X.Y] milestone files" .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md .planning/ROADMAP.md
```
...
**If doesn't exist:** Create from template at `~/.claude/get-things-done/templates/retrospective.md`.
...
**Commit:**
```bash
rapidx-sdk query commit "docs: update retrospective for v${VERSION}" .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(rapidx-sdk query init.execute-phase "1")
...
Detect base branch:
```bash
BASE_BRANCH=$(rapidx-sdk query config-get git.base_branch 2>/dev/null || echo "")
...
- [ ] User knows next step (/rapidx:new-milestone)
