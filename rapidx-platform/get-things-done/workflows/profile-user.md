...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `rapidx-sdk query` handlers (with legacy `rapidx-tools.cjs` parity where needed) and the rapidx-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
```bash
PROFILE_PATH="$HOME/.claude/get-things-done/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/get-things-done/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/get-things-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(rapidx-sdk query scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(rapidx-sdk query profile-sample --json 2>/dev/null)
```
...
**Spawn rapidx-user-profiler agent using Task tool:**

Use the Task tool to spawn the `rapidx-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/get-things-done/references/user-profiling.md`
...
Reference: @$HOME/.claude/get-things-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(rapidx-sdk query profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(rapidx-sdk query profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
rapidx-sdk query write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/get-things-done/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/get-things-done/USER-PROFILE.md" and jump to step 10.
...
```bash
rapidx-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
rapidx-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any `rapidx-sdk query` or rapidx-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
- [ ] Artifacts generated sequentially via rapidx-sdk query (or rapidx-tools.cjs) subcommands
