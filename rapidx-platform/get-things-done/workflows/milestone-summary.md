...
4. If nothing found: error "No milestone found. Run /rapidx:new-project or /rapidx:new-milestone first."
...
```bash
rapidx-sdk query init.progress
```
...
```bash
rapidx-sdk query commit "docs(v${VERSION}): generate milestone summary for onboarding" \
...
If the user is done:
- Suggest next steps: `/rapidx:new-milestone`, `/rapidx:progress`, or sharing the summary with the team
