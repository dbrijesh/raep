<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates rapidx-phase-researcher, rapidx-planner, and rapidx-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@~/.claude/get-things-done/references/revision-loop.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@~/.claude/get-things-done/references/tdd.md` so gate enforcement rules are available during planning.
...
```bash
PHASE_INFO=$(rapidx-sdk query roadmap.get-phase "${PHASE}")
```
...
5. Commit:
```bash
rapidx-sdk query commit "docs(${padded_phase}): generate context from PRD" "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(rapidx-sdk query config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
AI_PHASE_CFG=$(rapidx-sdk query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
### Spawn rapidx-phase-researcher

```bash
PHASE_DESC=$(rapidx-sdk query roadmap.get-phase "${PHASE}" --pick section)
```
...
Task(
  ...
  subagent_type="rapidx-phase-researcher",
...
**If found:**
1. Read template: `~/.claude/get-things-done/templates/VALIDATION.md`
...
SECURITY_ASVS=$(rapidx-sdk query config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(rapidx-sdk query config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(rapidx-sdk query config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(rapidx-sdk query config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(rapidx-sdk query roadmap.get-phase "${PHASE}" 2>/dev/null)
...
```bash
PHASE_SECTION=$(rapidx-sdk query roadmap.get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
## 7.8. Spawn rapidx-pattern-mapper Agent (Optional)
...
Check config:
```bash
PATTERN_MAPPER_CFG=$(rapidx-sdk query config-get workflow.pattern_mapper 2>/dev/null || echo "true")
```
...
Task(
  prompt="{above}",
  subagent_type="rapidx-pattern-mapper",
  model="{researcher_model}",
)
```
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @~/.claude/get-things-done/references/tdd.md to all eligible tasks:
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(rapidx-sdk query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$(rapidx-sdk query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```
...
5. **Re-run plan checker on bounced plans:** Spawn rapidx-plan-checker (same as step 10) on all modified plans. If a bounced plan fails the checker, restore original from its pre-bounce.md backup:
```
...
```bash
rapidx-sdk query commit "refactor(${padded_phase}): bounce plans through external refinement" "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
rapidx-sdk query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  rapidx-sdk query config-set workflow._auto_chain_active true
fi
```
...
- [ ] rapidx-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] rapidx-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] rapidx-plan-checker spawned with CONTEXT.md
