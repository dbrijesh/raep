...
AGENT_SKILLS_AUDITOR=$(rapidx-sdk query agent-skills rapidx-nyquist-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(rapidx-sdk query resolve-model rapidx-nyquist-auditor --raw)
...
## 5. Spawn rapidx-nyquist-auditor
...
Task(
  ...
  subagent_type="rapidx-nyquist-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/get-things-done/templates/VALIDATION.md`
...
rapidx-sdk query commit "docs(phase-${PHASE}): add/update validation strategy"
```
