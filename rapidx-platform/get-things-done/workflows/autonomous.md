...
```bash
DETAIL=$(rapidx-sdk query roadmap.get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(rapidx-sdk query init.phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(rapidx-sdk query config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
rapidx-sdk query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(rapidx-sdk query roadmap.get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
UI_REVIEW_CFG=$(rapidx-sdk query config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
Read and execute: `$HOME/.claude/get-things-done/references/autonomous-smart-discuss.md`
