...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" init phase-op "${PHASE}")
...
If "Skip": Exit with message: "Existing SPEC.md unchanged. Run /rapidx:discuss-phase [X] to continue."
...
Use the SPEC.md template from @~/.claude/get-things-done/templates/spec.md.
...
- User directed to /rapidx:discuss-phase as next step
</success_criteria>
