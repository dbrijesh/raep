...
```bash
UI_AUDITOR_MODEL=$(rapidx-sdk query resolve-model rapidx-ui-auditor --raw)
```
...
Task(
  ...
  subagent_type="rapidx-ui-auditor",
...
```bash
rapidx-sdk query commit "docs(${padded_phase}): UI audit review" "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
...
- [ ] rapidx-ui-auditor spawned with correct context
