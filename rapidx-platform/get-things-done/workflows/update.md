...
# runtime directories.
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/get-things-done/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/get-things-done/workflows/update.md" ]; }; then
  ...
  if [ -f "$PREFERRED_CONFIG_DIR/get-things-done/VERSION" ] && grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+' "$PREFERRED_CONFIG_DIR/get-things-done/VERSION"; then
    INSTALLED_VERSION="$(cat "$PREFERRED_CONFIG_DIR/get-things-done/VERSION")"
  else
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/get-things-done/VERSION" ] || [ -f "./$dir/get-things-done/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/get-things-done/VERSION"
    LOCAL_MARKER_FILE="./$dir/get-things-done/workflows/update.md"
...
for entry in "${ORDERED_ENV_RUNTIME_DIRS[@]}"; do
  ...
  if [ -f "$dir/get-things-done/VERSION" ] || [ -f "$dir/get-things-done/workflows/update.md" ]; then
    ...
    GLOBAL_VERSION_FILE="$dir/get-things-done/VERSION"
    GLOBAL_MARKER_FILE="$dir/get-things-done/workflows/update.md"
...
if [ -z "$GLOBAL_RUNTIME" ]; then
  for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
    runtime="${entry%%:*}"
    dir="${entry#*:}"
    if [ -f "$HOME/$dir/get-things-done/VERSION" ] || [ -f "$HOME/$dir/get-things-done/workflows/update.md" ]; then
      GLOBAL_RUNTIME="$runtime"
      GLOBAL_VERSION_FILE="$HOME/$dir/get-things-done/VERSION"
      GLOBAL_MARKER_FILE="$HOME/$dir/get-things-done/workflows/update.md"
...
Check for rapidx-local-patches/backup-meta.json in the config directory.
