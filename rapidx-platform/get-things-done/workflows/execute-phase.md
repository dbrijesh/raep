...
- **Claude Code:** Uses `Task(subagent_type="rapidx-executor", ...)` — blocks until complete, returns result
...
@~/.claude/get-things-done/references/agent-contracts.md
@~/.claude/get-things-done/references/context-budget.md
@~/.claude/get-things-done/references/gates.md
</required_reading>
...
- rapidx-verifier — Verifies phase completion, checks quality gates
...
- rapidx-integration-checker — Checks cross-phase integration
...
```bash
INIT=$(rapidx-sdk query init.execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(rapidx-sdk query agent-skills rapidx-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(rapidx-sdk query config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @~/.claude/get-things-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @~/.claude/get-things-done/references/planner-antipatterns.md
...
Check if the current runtime is Copilot by testing for the `@rapidx-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  rapidx-sdk query config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/get-things-done/workflows/execute-plan.md` **inline**
...
```bash
rapidx-sdk query state.begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(rapidx-sdk query phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
CROSS_AI_ENABLED=$(rapidx-sdk query config-get workflow.cross_ai_execution 2>/dev/null || echo "false")
CROSS_AI_CMD=$(rapidx-sdk query config-get workflow.cross_ai_command 2>/dev/null || echo "")
CROSS_AI_TIMEOUT=$(rapidx-sdk query config-get workflow.cross_ai_timeout 2>/dev/null || echo "300")
```
...
`workflow.cross_ai_command` via `rapidx-sdk query config-set workflow.cross_ai_command "<command>"`.
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       For `rapidx-sdk query commit` (or legacy `rapidx-tools.cjs` commit): add --no-verify flag when needed.
       ...
       @~/.claude/get-things-done/templates/summary.md
       @~/.claude/get-things-done/references/checkpoints.md
       @~/.claude/get-things-done/references/tdd.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@~/.claude/get-things-done/references/executor-examples.md'}
       </execution_context>
   ...
   for WT in $WORKTREES; do
     ...
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ]; then
       ...
       if ! git diff --quiet .planning/STATE.md .planning/ROADMAP.md 2>/dev/null || \
         ...
         COMMIT_DOCS=$(rapidx-sdk query config-get commit_docs 2>/dev/null || echo "true")
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       rapidx-sdk query roadmap.update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done
     ...
     if ! git diff --quiet .planning/ROADMAP.md .planning/STATE.md 2>/dev/null; then
       rapidx-sdk query commit "docs(phase-${PHASE_NUMBER}): update tracking after wave ${N}" .planning/ROADMAP.md .planning/STATE.md
     fi
   elif [ "${TEST_EXIT}" -eq 124 ]; then
...
7b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    rapidx-sdk query verify.key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(rapidx-sdk query config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(rapidx-sdk query config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(rapidx-sdk query phase-plan-index "${PHASE_NUMBER}")
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(rapidx-sdk query config-get workflow.code_review 2>/dev/null || echo "true")
```
...
```bash
PARENT_INFO=$(rapidx-sdk query find-phase "${PARENT_PHASE}" --raw)
...
```bash
rapidx-sdk query commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
SCHEMA_DRIFT=$(rapidx-sdk query verify.schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
```bash
VERIFIER_SKILLS=$(rapidx-sdk query agent-skills rapidx-verifier 2>/dev/null)
```
...
Commit the file:
```bash
rapidx-sdk query commit "test({phase_num}): persist human verification items as UAT" "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
COMPLETION=$(rapidx-sdk query phase.complete "${PHASE_NUMBER}")
```
...
```bash
rapidx-sdk query commit "docs(phase-{X}): complete phase execution" .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
**Check config gate:**
```bash
GL_ENABLED=$(rapidx-sdk query config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
rapidx-sdk query learnings.copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
```bash
rapidx-sdk query commit "docs(phase-{X}): evolve PROJECT.md after phase completion" .planning/PROJECT.md
```
...
Read and follow `~/.claude/get-things-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
