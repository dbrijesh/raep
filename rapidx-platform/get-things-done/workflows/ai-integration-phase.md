...
<required_reading>
@~/.claude/get-things-done/references/ai-frameworks.md
...
```bash
SELECTOR_MODEL=$(rapidx-sdk query resolve-model rapidx-framework-selector 2>/dev/null | jq -r '.model' 2>/dev/null || true)
RESEARCHER_MODEL=$(rapidx-sdk query resolve-model rapidx-ai-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
DOMAIN_MODEL=$(rapidx-sdk query resolve-model rapidx-domain-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
PLANNER_MODEL=$(rapidx-sdk query resolve-model rapidx-eval-planner 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Check config:
```bash
AI_PHASE_ENABLED=$(rapidx-sdk query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
## 5. Spawn rapidx-framework-selector
...
Copy template:
```bash
cp "$HOME/.claude/get-things-done/templates/AI-SPEC.md" "${PHASE_DIR}/${PADDED_PHASE}-AI-SPEC.md"
```
...
## 8. Spawn rapidx-domain-researcher
