...
```bash
INIT=$(rapidx-sdk query init.progress)
...
Run /rapidx:new-project to start a new project.
```

Exit.

If missing STATE.md: suggest `/rapidx:new-project`.

**If ROADMAP.md missing but PROJECT.md exists:**

This means a milestone was completed and archived. Go to **Route F** (between milestones).

If missing both ROADMAP.md and PROJECT.md: suggest `/rapidx:new-project`.
</step>

<step name="load">
**Use structured extraction from `rapidx-sdk query` (or legacy rapidx-tools.cjs):**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(rapidx-sdk query roadmap.analyze)`
- `STATE=$(rapidx-sdk query state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  rapidx-sdk query summary-extract <path> --fields one_liner
  ```
...
<step name="report">
**Generate progress bar from `rapidx-sdk query progress` / `progress.json`, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(rapidx-sdk query progress.bar --raw)
```
...
## Pending Todos
- [count] pending — /rapidx:check-todos to review

## Active Debug Sessions
- [count] active — /rapidx:debug to continue
...
```bash
DEBT=$(rapidx-sdk query audit-uat --raw 2>/dev/null)
```
...
Review: `/rapidx:audit-uat ${RAPIDX_WS}` — full cross-phase audit
Resume testing: `/rapidx:verify-work {phase} ${RAPIDX_WS}` — retest specific phase
```
...
```bash
PHASE_SECTION=$(rapidx-sdk query roadmap.get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/rapidx:plan-phase {phase-number} ${RAPIDX_WS}`
...
`/rapidx:discuss-phase {phase}` — gather context and clarify approach

---

**Also available:**
- `/rapidx:ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/rapidx:plan-phase {phase}` — skip discussion, plan directly
- `/rapidx:list-phase-assumptions {phase}` — see Claude's assumptions
...
`/rapidx:discuss-phase {phase} ${RAPIDX_WS}` — gather context and clarify approach

---

**Also available:**
- `/rapidx:plan-phase {phase} ${RAPIDX_WS}` — skip discussion, plan directly
- `/rapidx:list-phase-assumptions {phase} ${RAPIDX_WS}` — see Claude's assumptions
...
`/rapidx:plan-phase {phase} --gaps ${RAPIDX_WS}`

---

**Also available:**
- `/rapidx:execute-phase {phase} ${RAPIDX_WS}` — execute phase plans
- `/rapidx:verify-work {phase} ${RAPIDX_WS}` — run more UAT testing
...
`/rapidx:verify-work {phase} ${RAPIDX_WS}` — resume testing from where you left off

---

**Also available:**
- `/rapidx:audit-uat ${RAPIDX_WS}` — full cross-phase UAT audit
- `/rapidx:execute-phase {phase} ${RAPIDX_WS}` — execute phase plans
...
```bash
NEXT_PHASE_SECTION=$(rapidx-sdk query roadmap.get-phase "$((Z+1))" 2>/dev/null)
...
`/rapidx:discuss-phase {Z+1}` — gather context and clarify approach

---

**Also available:**
- `/rapidx:ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
- `/rapidx:plan-phase {Z+1}` — skip discussion, plan directly
- `/rapidx:verify-work {Z}` — user acceptance test before continuing
...
`/rapidx:discuss-phase {Z+1} ${RAPIDX_WS}` — gather context and clarify approach

---

**Also available:**
- `/rapidx:plan-phase {Z+1} ${RAPIDX_WS}` — skip discussion, plan directly
- `/rapidx:verify-work {Z} ${RAPIDX_WS}` — user acceptance test before continuing
...
`/rapidx:complete-milestone ${RAPIDX_WS}`

---

**Also available:**
- `/rapidx:verify-work ${RAPIDX_WS}` — user acceptance test before completing milestone
...
`/rapidx:new-milestone ${RAPIDX_WS}`
...
- Phase complete but next phase not planned → offer `/rapidx:plan-phase [next] ${RAPIDX_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/rapidx:resume-work ${RAPIDX_WS}`
</step>
...
- Check 2 (orphaned handoff): `Read the handoff file(s) and resume from where work was paused: /rapidx:resume-work ${RAPIDX_WS}`
...
- Check 1 (STATE inconsistency): `Run /rapidx:verify-work ${PHASE} ${RAPIDX_WS} to reconcile state`
</step>
...
- [ ] Smart routing: /rapidx:execute-phase if plans exist, /rapidx:plan-phase if not
