...
<required_reading>
@~/.claude/get-things-done/references/continuation-format.md
</required_reading>
...
```bash
INIT=$(rapidx-sdk query init.resume)
...
**If `planning_exists` is false:** This is a new project - route to /rapidx:new-project
</step>
...
- This is the primary resumption source — structured data from `/rapidx:pause-work`
...
📋 [N] pending todos — /rapidx:check-todos to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/rapidx:execute-phase {phase} ${RAPIDX_WS})
   OR
1. Discuss Phase 3 context (/rapidx:discuss-phase 3 ${RAPIDX_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/rapidx:plan-phase 3 ${RAPIDX_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show command for user to run after clearing:
  ...
  **Also available:**
  - `/rapidx:discuss-phase [N] ${RAPIDX_WS}` — gather context first
  - `/rapidx:research-phase [N] ${RAPIDX_WS}` — investigate unknowns
