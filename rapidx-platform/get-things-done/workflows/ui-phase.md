<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates rapidx-ui-researcher and rapidx-ui-checker with a revision loop. Inserts between discuss-phase and plan-phase in the lifecycle.
...
AGENT_SKILLS_UI=$(rapidx-sdk query agent-skills rapidx-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(rapidx-sdk query agent-skills rapidx-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(rapidx-sdk query resolve-model rapidx-ui-researcher --raw)
UI_CHECKER_MODEL=$(rapidx-sdk query resolve-model rapidx-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(rapidx-sdk query config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
Template: ~/.claude/get-things-done/templates/UI-SPEC.md
</output>
...
Task(
  ...
  subagent_type="rapidx-ui-researcher",
...
Task(
  ...
  subagent_type="rapidx-ui-checker",
...
- Re-spawn rapidx-ui-researcher with revision context:
...
```bash
rapidx-sdk query commit "docs(${padded_phase}): UI design contract" "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
...
- [ ] rapidx-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] rapidx-ui-checker spawned with UI-SPEC.md
