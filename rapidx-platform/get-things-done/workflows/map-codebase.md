...
```bash
INIT=$(rapidx-sdk query init.map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(rapidx-sdk query agent-skills rapidx-codebase-mapper 2>/dev/null)
```
...
Spawn 4 parallel rapidx-codebase-mapper agents.

Use Task tool with `subagent_type="rapidx-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `rapidx-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="rapidx-codebase-mapper",
  model="{mapper_model}",
...
Use the same document templates as the `rapidx-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
rapidx-sdk query commit "docs: map existing codebase" .planning/codebase/*.md
```
...
- If Task tool available: 4 parallel rapidx-codebase-mapper agents spawned with run_in_background=true
