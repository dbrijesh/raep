...
```bash
rapidx-sdk query phases.clear --confirm
```

```bash
rapidx-sdk query commit "docs: start milestone v[X.Y] [Name]" .planning/PROJECT.md .planning/STATE.md
```
...
Spawn 4 parallel rapidx-project-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: ~/.claude/get-things-done/templates/research-project/{FILE}
</output>
", subagent_type="rapidx-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
rapidx-sdk query commit "docs: define milestone v[X.Y] requirements" .planning/REQUIREMENTS.md
```
...
```bash
rapidx-sdk query commit "docs: create milestone v[X.Y] roadmap ([N] phases)" .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
- [ ] rapidx-roadmapper spawned with phase numbering context
