<purpose>
Remove a RapidX workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(rapidx-sdk query init.remove-workspace "$WORKSPACE_NAME")
...
First run `/rapidx:list-workspaces` to show available workspaces, then ask:
