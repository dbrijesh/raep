...
Seeds auto-surface during /rapidx:new-milestone when trigger conditions match the
...
This seed should be presented during `/rapidx:new-milestone` when the milestone
...
```bash
rapidx-sdk query commit "docs: plant seed — {$IDEA}" .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /rapidx:new-milestone
