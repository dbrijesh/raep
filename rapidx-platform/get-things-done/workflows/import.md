...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via rapidx-plan-checker
...
Delegate validation to rapidx-plan-checker:
...
```bash
rapidx-sdk query commit "docs({phase}): import plan from {basename FILEPATH}" .planning/phases/{phase}/{plan}-PLAN.md .planning/ROADMAP.md
```
