<purpose>
Analyze ROADMAP.md phases for dependency relationships before execution. Detect file overlap between phases, semantic API/data-flow dependencies, and suggest `Depends on` entries to prevent merge conflicts during parallel execution by `/rapidx:manager`.
</purpose>
...
Read `.planning/ROADMAP.md`. If it does not exist, error: "No ROADMAP.md found — run `/rapidx:new-project` first."
...
After applying: "ROADMAP.md updated. Run `/rapidx:manager` to execute phases in the correct order."
