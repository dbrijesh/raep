<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns rapidx-code-reviewer agent, commits REVIEW.md, and presents results to user.
</purpose>
...
- rapidx-code-reviewer: Reviews source files for bugs and quality issues
</available_agent_types>
...
```
Error: Phase ${PHASE_ARG} not found. Run /rapidx:status to see available phases.
```
...
2. Config value: `rapidx-sdk query config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(rapidx-sdk query config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /rapidx:code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
Spawn the rapidx-code-reviewer agent:

```
Task(subagent_type="rapidx-code-reviewer", prompt="
<files_to_read>
${FILES_TO_READ}
</files_to_read>
...
No REVIEW.md created. You can retry with /rapidx:code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with /rapidx:code-review ${PHASE_ARG}"
fi
```
</step>
...
Next steps:
  /rapidx:code-review-fix ${PHASE_NUMBER}  — Auto-fix issues
