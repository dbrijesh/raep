...
<template>
@~/.claude/get-things-done/templates/UAT.md
</template>
...
```
UI_PHASE_FLAG=$(rapidx-sdk query config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(rapidx-sdk query uat.render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
rapidx-sdk query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
Read and follow `~/.claude/get-things-done/workflows/transition.md`.
...
`audit-open` is CJS-only until registered on `rapidx-sdk query`:

```bash
node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" audit-open --json 2>/dev/null
```
...
- Follow @~/.claude/get-things-done/workflows/diagnose-issues.md
...
Spawn rapidx-planner in --gaps mode:
...
Spawn rapidx-planner with revision context:
