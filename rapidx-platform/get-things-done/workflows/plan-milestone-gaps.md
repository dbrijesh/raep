<purpose>
Create all phases necessary to close gaps identified by `/rapidx:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/rapidx:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/rapidx:audit-milestone` first.
```
...
HIGHEST=$(rapidx-sdk query phases.list --pick directories[-1])
```
...
```bash
rapidx-sdk query commit "docs(roadmap): add gap closure phases {N}-{M}" .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/rapidx:execute-phase {N}` — if plans already exist
...
`/rapidx:audit-milestone` — re-audit to verify gaps closed
`/rapidx:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/rapidx:plan-phase` next
</success_criteria>
