...
```bash
# Get state snapshot
rapidx-sdk query state.json 2>/dev/null || echo "{}"
```
...
After passing all three hard-stop gates, scan all phases that precede the current phase in ROADMAP.md order for incomplete work. For each prior phase number `N`, use `rapidx-sdk query find-phase <N>` JSON (plans, summaries, incomplete_plans, etc.) to inspect that phase.
...
```bash
rapidx-sdk query commit "docs: defer incomplete Phase {src} items to backlog"
```
