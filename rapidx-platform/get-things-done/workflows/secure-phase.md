...
AGENT_SKILLS_AUDITOR=$(rapidx-sdk query agent-skills rapidx-security-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(rapidx-sdk query resolve-model rapidx-security-auditor --raw)
...
## 5. Spawn rapidx-security-auditor
...
Task(
  ...
  subagent_type="rapidx-security-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/get-things-done/templates/SECURITY.md`
...
```bash
rapidx-sdk query commit "docs(phase-${PHASE}): add/update security threat verification"
```
