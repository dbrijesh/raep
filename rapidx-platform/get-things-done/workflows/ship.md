...
```bash
CONFIG=$(rapidx-sdk query state.load)
```
...
```bash
REVIEW_CMD=$(rapidx-sdk query config-get workflow.code_review_command 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(rapidx-sdk query state.load 2>/dev/null | head -20)
   ```
...
```bash
rapidx-sdk query state.update "Last Activity" "$(date +%Y-%m-%d)"
rapidx-sdk query state.update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
rapidx-sdk query commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" .planning/STATE.md
```
</step>
