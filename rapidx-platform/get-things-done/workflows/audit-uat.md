...
```bash
AUDIT=$(rapidx-sdk query audit-uat --raw)
```
...
1. **Close stale items:** `/rapidx:verify-work {phase}` — mark stale tests as resolved
2. **Run active tests:** Human UAT test plan below
3. **When prerequisites met:** Retest blocked items with `/rapidx:verify-work {phase}`
```
</step>
