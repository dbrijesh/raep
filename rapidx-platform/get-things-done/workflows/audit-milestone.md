...
```bash
INIT=$(rapidx-sdk query init.milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_CHECKER=$(rapidx-sdk query agent-skills rapidx-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(rapidx-sdk query resolve-model rapidx-integration-checker --raw)
```
...
PHASE_INFO=$(rapidx-sdk query find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="rapidx-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  rapidx-sdk query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(rapidx-sdk query config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/rapidx:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/rapidx:validate-phase {N}` |

Phases needing validation: run `/rapidx:validate-phase {N}` for each flagged phase.
...
- /rapidx:complete-milestone {version} — proceed anyway (accept tech debt)
