...
**Delegate ROADMAP.md and STATE.md updates to `rapidx-sdk query phase.complete`:**

```bash
TRANSITION=$(rapidx-sdk query phase.complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `rapidx-sdk query phase.complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(rapidx-sdk query progress.bar --raw)
```
...
**Use the transition result from `rapidx-sdk query phase.complete`:**
...
```bash
rapidx-sdk query config-set workflow._auto_chain_active false
```
