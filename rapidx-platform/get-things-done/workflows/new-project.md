...
```bash
mkdir -p .planning
rapidx-sdk query config-new-project '{"mode":"yolo","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":true|false,"auto_advance":true}}'
```
...
```bash
mkdir -p .planning
rapidx-sdk query commit "chore: add project config" .planning/config.json
```
...
```bash
mkdir -p .planning
rapidx-sdk query commit "docs: initialize project" .planning/PROJECT.md
```
...
Spawn 4 parallel rapidx-project-researcher agents with path references:
...
Use template: ~/.claude/get-things-done/templates/research-project/STACK.md
</output>
", subagent_type="rapidx-project-researcher", model="{researcher_model}", description="Stack research")
...
Use template: ~/.claude/get-things-done/templates/research-project/FEATURES.md
</output>
", subagent_type="rapidx-project-researcher", model="{researcher_model}", description="Features research")
...
Use template: ~/.claude/get-things-done/templates/research-project/ARCHITECTURE.md
</output>
", subagent_type="rapidx-project-researcher", model="{researcher_model}", description="Architecture research")
...
Use template: ~/.claude/get-things-done/templates/research-project/PITFALLS.md
</output>
", subagent_type="rapidx-project-researcher", model="{researcher_model}", description="Pitfalls research")
```
...
Use template: ~/.claude/get-things-done/templates/research-project/SUMMARY.md
Commit after writing.
</output>
", subagent_type="rapidx-research-synthesizer", model="{synthesizer_model}", description="Synthesize research")
```
...
```bash
rapidx-sdk query commit "docs: define v1 requirements" .planning/REQUIREMENTS.md
```
...
Spawn rapidx-roadmapper agent with path references:
...
", subagent_type="rapidx-roadmapper", model="{roadmapper_model}", description="Create roadmap")
```
...
- Re-spawn roadmapper with revision context:
  ...
  ", subagent_type="rapidx-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
```bash
rapidx-sdk query commit "docs: create roadmap ([N] phases)" .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
```bash
PHASE1_SECTION=$(rapidx-sdk query roadmap.get-phase 1 2>/dev/null)
...
- [ ] rapidx-roadmapper spawned with context
