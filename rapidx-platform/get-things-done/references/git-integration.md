...
```bash
rapidx-sdk query commit "docs: initialize [project-name] ([N] phases)" .planning/
```
...
```bash
rapidx-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
rapidx-sdk query commit "wip: [phase-name] paused at task [X]/[Y]" .planning/
```
...
```bash
rapidx-sdk query commit-to-subrepo "feat(02-01): add user API" \
