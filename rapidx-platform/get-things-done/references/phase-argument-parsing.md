...
```bash
PHASE_INFO=$(rapidx-sdk query find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(rapidx-sdk query roadmap.get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(rapidx-sdk query find-phase "${PHASE}" --raw)
```
