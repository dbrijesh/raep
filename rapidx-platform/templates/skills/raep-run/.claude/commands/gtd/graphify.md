...
**CJS-only (graphify):** `graphify` subcommands are not registered on `rapidx-sdk query`. Use `node $HOME/.claude/get-shit-done/bin/rapidx:tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `rapidx-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node $HOME/.claude/get-shit-done/bin/rapidx:tools.cjs config-set graphify.enabled true
...
```bash
node $HOME/.claude/get-shit-done/bin/rapidx:tools.cjs graphify query <term>
```
...
```bash
node $HOME/.claude/get-shit-done/bin/rapidx:tools.cjs graphify status
```
...
```bash
node $HOME/.claude/get-shit-done/bin/rapidx:tools.cjs graphify diff
```
...
```
PREFLIGHT=$(node "$HOME/.claude/get-shit-done/bin/rapidx:tools.cjs" graphify build)
```
...
Project root: ${CWD}
gsd-tools path: $HOME/.claude/get-shit-done/bin/rapidx:tools.cjs
...
4. **Write diff snapshot:**
   ```
   node \"$HOME/.claude/get-shit-done/bin/rapidx:tools.cjs\" graphify build snapshot
   ```
