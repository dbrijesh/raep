...
1. Generate slug from description:
   ```bash
   SLUG=$(node "$HOME/.claude/get-things-done/bin/gsd-tools.cjs" generate-slug "$ARGUMENTS")
   ```
...
5. Commit:
   ```bash
   node "$HOME/.claude/get-things-done/bin/gsd-tools.cjs" commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
