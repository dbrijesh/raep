...
function installPlugin(pluginName, projectRoot, options = {}) {
  ...
  // Register plugin components into stack.json so the next install/health run
  // picks them up and copies the actual skill/agent/hook files to each IDE.
  const stackPath = path.join(projectRoot, '.rapidx', 'stack.json');
  try {
    const stack = JSON.parse(fs.readFileSync(stackPath, 'utf8'));
    const ic = stack.installed_components || { rules: [], skills: [], agents: [], hooks: [] };
    const c = plugin.components || {};
    const merge = (arr, additions) => Array.from(new Set([...(arr || []), ...(additions || [])]));
    stack.installed_components = {
      rules: merge(ic.rules, c.rules),
      skills: merge(ic.skills, c.skills),
      agents: merge(ic.agents, c.agents),
      hooks: merge(ic.hooks, c.hooks),
    };
    if (!stack.plugins) stack.plugins = [];
    if (!stack.plugins.includes(pluginName)) stack.plugins.push(pluginName);
    stack.updated_at = new Date().toISOString();
    fs.writeFileSync(stackPath, JSON.stringify(stack, null, 2), 'utf8');
  } catch (e) {
    process.stderr.write(`[RapidX] Could not update stack.json for plugin '${pluginName}': ${e.message}\n`);
    process.stderr.write(`[RapidX] Re-run the installer to apply plugin components to your IDE configs.\n`);
  }
