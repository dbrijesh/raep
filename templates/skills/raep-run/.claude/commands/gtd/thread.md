...
```
No threads found. Create one with: /rapidx:thread <description>
```
</mode_list>
...
1. Generate slug from description:
   ```bash
   SLUG=$(node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" generate-slug "$ARGUMENTS")
   ```
...
5. Commit:
   ```bash
   node "$HOME/.claude/get-things-done/bin/rapidx-tools.cjs" commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
...
6. Report:
   ...
   Resume anytime with: /rapidx:thread {slug}
   ```
</mode_create>
...
- Lighter weight than /rapidx:pause-work — no phase state, no plan context
