...
function installAntigravity(options) {
  ...
  const commandsDir = path.join(agentDir, 'commands', 'rapidx');
  ...
  // Install agent definition files to .agent/agents/
  ...
  for (const agentName of requestedAgents.filter(a => AGENT_NAMES.includes(a))) {
    ...
    const augmented = injectAgentSkills(raw, agentName, installedSkills, 'antigravity');
  ...
  if (agentCount > 0) {
    process.stdout.write(`  [RapidX] Installed ${agentCount} agent definitions → .agent/agents/\n`);
  }

  // Generate Get Things Done command files
  ...
  if (gtdResult.generated > 0) {
    process.stdout.write(`  [RapidX] Generated ${gtdResult.generated} Get Things Done commands → .agent/commands/rapidx/\n`);
  }
